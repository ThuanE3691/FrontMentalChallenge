...
const EventsArea = ({ user, currentUser }) => {
	return currentUser.username === user.username ? (
		<>
			...
			<div className="flex items-center cursor-pointer w-fit text-comments-page-Moderate-blue hover:text-comments-page-Light-grayish-blue color_trans">
		...
		<div className="flex items-center cursor-pointer text-comments-page-Moderate-blue hover:text-comments-page-Light-grayish-blue color_trans">
	...
	return (
		<section
			className={`grid md:grid-rows-[auto_auto] overflow-hidden grid-rows-[auto_auto_auto] grid-cols-[auto_repeat(3,1fr)] bg-white rounded-xl px-6 py-5 gap-x-4 gap-y-4 ${
				commentType === "reply" ? "col-start-2 " : "md:w-[712px]"
			}`}
		>
			...
			<div className="flex items-center self-center col-start-4 row-start-3 font-medium md:self-start md:col-start-4 md:row-start-1 md:mt-1 place-self-end ">
				<EventsArea user={comment.user} currentUser={currentUser}></EventsArea>
			</div>
